...
class OrderManagerAgent extends BaseAgent {
   ...
   */
  async createOrder({ businessId, customerPhone, customerName, deliveryAddress, items, subtotal, grandTotal, paymentMethod, source, eventService }) {
    console.log('\n' + '▓'.repeat(80));
   ...
   * businessId: string or ObjectId (will be cast). status: single value or comma-separated (e.g. "accepted,preparing,out_for_delivery").
   */
  async listOrders({ businessId, status, limit = 20, page = 1 }) {
    if (!businessId) {
      return { success: true, data: { orders: [], total: 0, page: 1, pages: 0 }, responseText: 'No business context.' };
    }
    const query = {
